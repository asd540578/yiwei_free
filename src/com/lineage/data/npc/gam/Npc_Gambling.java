...
public class Npc_Gambling extends NpcExecutor {
	...
	public void talk(final L1PcInstance pc, final L1NpcInstance npc) {
		...
		if (gambling == null) {
			...
			for (Integer key : GamblingTime.get_gambling().get_allNpc().keySet()) {
				 GamblingNpc o = GamblingTime.get_gambling().get_allNpc().get(key);
	...
	public void action(final L1PcInstance pc, final L1NpcInstance npc, final String cmd, final long amount) {
		...
		if (cmd.equals("a")) {// 賣中獎的票
			sell(pc, npc);

		} else if (cmd.equals("b")) {// 下注本場
			pc.sendPackets(new S_ShopSellListGam(pc, npc));

		} else if (cmd.equals("c")) {// 查看紀錄
			reading(pc, npc);
		}
	...
	private void reading(final L1PcInstance pc, final L1NpcInstance npc) {
		...
		for (final GamblingNpc gamblingNpc : npclist.values()) {
			...
			final StringBuilder adena = RangeLong.scount(gamblingNpc.get_adena());
	...
	private void sell(final L1PcInstance pc, final L1NpcInstance npc) {
		...
		//System.out.println("賣中獎的票");
		final L1ItemInstance[] items = pc.getInventory().findItemsId(40309);// 食人妖精競賽票
		...
		for (final L1ItemInstance item : items) {
			final L1Gambling gam = GamblingReading.get().getGambling(item.getGamNo());
			if (gam != null) {
