...
public class RankingHeroTimer extends TimerTask {
	...
	private static void load() {
		try {
			...
			for (final Iterator<L1PcInstance> iter = allPc.iterator(); iter.hasNext();) {
				...
				if (count > 0) {
					if (tgpc.isCrown()) {
						_userNameC = intTree(count, tgpc.getName(), _userNameC);
						
					} else if (tgpc.isKnight()) {
						_userNameK = intTree(count, tgpc.getName(), _userNameK);
						
					} else if (tgpc.isElf()) {
						_userNameE = intTree(count, tgpc.getName(), _userNameE);
						
					} else if (tgpc.isWizard()) {
						_userNameW = intTree(count, tgpc.getName(), _userNameW);
						
					} else if (tgpc.isDarkelf()) {
						_userNameD = intTree(count, tgpc.getName(), _userNameD);
						
					} else if (tgpc.isDragonKnight()) {
						_userNameG = intTree(count, tgpc.getName(), _userNameG);
						
					} else if (tgpc.isIllusionist()) {
						_userNameI = intTree(count, tgpc.getName(), _userNameI);
	 ...
	 * @return
	 */
	private static String[] intTree(final int level, final String name, final String[] userName) {
		if (userName[0].equals(" ")) {// NO 1
			userName[0] = name + "," + level;
			return userName;
			
		} else {
			final String[] set = userName[0].split(",");
			final int srcLevel = Integer.parseInt(set[1]);
			if (srcLevel < level && !set[0].equals(name)) {
