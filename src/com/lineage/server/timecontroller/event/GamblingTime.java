...
public class GamblingTime extends TimerTask {
	...
	//private static final Map<Integer, Integer> _xmap = new ConcurrentHashMap<Integer, Integer>();
	...
	public void run() {
		try {
			...
			long previous = 0;
			/*final L1Gambling previouss = GamblingReading.get().getGambling(_gamblingNo - 1);
			
			if (previouss != null) {
				if (GamblingSet.NOWODDS > 0D) {
					// 上次賭金總數的15%
					previous = (long) Math.max((previouss.get_adena() * GamblingSet.NOWODDS), 0);
				}
			} else {
				previous = 1000000;// 無上次獎金資料 預設 100萬
			}

			if (previous <= 0) {
				previous = 1000000;// 上次獎金資料為0 預設 100萬
			}*/
			...
			for (final Iterator<L1PcInstance> iter = World.get().getAllPlayers().iterator(); iter.hasNext();) {
				...
				listner.sendPackets(new S_ServerMessage("奇岩競技場比賽即將開始"));
			}
			...
			if (_gambling.get_oneNpc() != null) {
				...
				for (final L1PcInstance listner : World.get().getAllPlayers()) {
					...
					listner.sendPackets(new S_ServerMessage(166, "$375 " + _gamblingNo + " $366 " + onename + 
							" " + GamblingSet.GAMADENATIME + ":" + (one.get_adena() * 2)));
				}
				
				_log.info("奇岩賭場:" + _gamblingNo + " 優勝者:" + one.get_npc().getName() + "/" +
						one.get_xId() + "-" + _gambling.WIN + " (" + one.get_adena() + ")");
				/*int value = 1;
				if (_xmap.get(one.get_xId()) != null) {
					value += _xmap.get(one.get_xId());
				}
				_xmap.put(one.get_xId(), value);
				for (Integer key : _xmap.keySet()) {
					_log.info(key + " (" + _xmap.get(key) + ")");
				}*/
				
				// 取回勝者紀錄
				final int npcid = _gambling.get_oneNpc().get_npc().getNpcId();
				// 獲勝NPC賠率
				final double rate = 2.00;//_gambling.get_oneNpc().get_rate();
				// 本場總下注金額
			...
			// 給予奇岩城稅收(給予大於0D)
			/*if (GamblingSet.GET_GIRAN_CASTLE > 0D) {
				// 上次賭金總數的2%
				long previous = (long) Math.max((gambling.get_adena() * GamblingSet.GET_GIRAN_CASTLE), 0);
				getCastle(previous);
			}*/
			_isStart = false;
		}
	 ...
	 * @param previous
	 */
	/*private void getCastle(long previous) {
		final L1Castle l1castle = CastleReading.get().getCastleTable(L1CastleLocation.GIRAN_CASTLE_ID);
			...
			case 0:
				if (_random.nextInt(100) < 20) {
					...
					for (final L1PcInstance listner : World.get().getAllPlayers()) {
						...
						listner.sendPackets(new S_KillMessage(npc.getNameId(), msg, 0));
					}
				}
				break;
			}
		...
		for (final L1Object obj : allObj) {
			...
			case 4:
				if (npcId != 91172) {
					...
					final int index = rates.indexOf(".");
					if (index != -1) {
						final int length = rates.length();
						if (length >= (index + 3)) {
							rates = rates.substring(0, index + 3);

						} else {
							rates = rates.substring(0, index + 2);
							rates += "0";
						}
					...
					npc.broadcastPacketX10(new S_NpcChatShouting(npc, npcname + " $402 2.0"/* + rates*/));
				}
				break;
