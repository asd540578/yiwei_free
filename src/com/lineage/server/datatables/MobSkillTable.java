	...
	private void loadMobSkillData() {
		...
		try {
			...
			pstm1 = con.prepareStatement(
					"SELECT mobid,count(*) as cnt FROM mobskill group by mobid");
			...
			for (rs1 = pstm1.executeQuery(); rs1.next();) {
				...
				try {
					...
					while (rs2.next()) {
						...
						mobskill.setType(actNo, rs2.getInt("type"));// 類型
						mobskill.setTriggerRandom(actNo, rs2.getInt("TriRnd"));// 施放機率
			...
			_log.info("載入MOB技能資料數量: " + _mobskills.size() + "(" + timer.get()
					+ "ms)");
