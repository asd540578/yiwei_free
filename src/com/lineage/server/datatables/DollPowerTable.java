...
public class DollPowerTable {
				...
				if (powers != null) {
					if (!powers.equals("")) {
						...
						for (final String string : set1) {
							L1DollExecutor e = _classList.get(Integer.parseInt(string));
							if (e != null) {
								powerList.add(e);
				...
				if (need != null) {
					if (!need.equals("")) {
						...
						for (int i = 0 ; i < set2.length ; i++) {
							...
							// 找回物品資訊
							final L1Item temp = 
								ItemTable.get().getTemplate(itemid_n);
							if (temp == null) {
	 ...
	 * @param int3
	 */
	private void addList(final int powerid, final String className, int int1, int int2, int int3) {
