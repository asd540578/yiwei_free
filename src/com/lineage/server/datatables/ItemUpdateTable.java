	...
	public void load() {
		...
		try {
			...
			while (rs.next()) {
				...
				final String needids_c = rs.getString("needids").replaceAll(" ", "");// 取代空白
				final String[] needids_tmp = needids_c.split(",");
				
				final String needcounts_c = rs.getString("needcounts").replaceAll(" ", "");// 取代空白
	...
	public static void delete(final int id) {
		...
		try {
			...
			ps = cn.prepareStatement(
					"DELETE FROM `server_item_update` WHERE `id`=?");
			ps.setInt(1, id);
			ps.execute();
