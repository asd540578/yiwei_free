	...
	public void load() {
		...
		try {
			...
			while (rs.next()) {
				...
				final double amount_rate = MapsTable.get().getMonsterAmount(rs.getShort("mapid"));
				...
				spawnDat.setRest(rs.getBoolean("rest"));// 召喚後動作暫停
	...
	public static void storeSpawn(final L1PcInstance pc, final L1Npc npc) {
		...
		try {
			...
			pstm = con.prepareStatement(
					"INSERT INTO `spawnlist` SET `location`=?,`count`=?," +
