...
public class AuctionBoardTable implements AuctionBoardStorage {
	...
	public void load() {
		...
		try {
			...
			while (rs.next()) {
				...
				if (!location.startsWith("$")) {
					...
					if ((houseId >= 458753) && (houseId <= 458819)) { // 亞丁
						townName = "$2129 ";
					}

					if ((houseId >= 524289) && (houseId <= 524294)) { // 古魯丁
						townName = "$381 ";
					}
		...
		_log.info("載入盟屋拍賣公告欄資料數量: " + _boards.size() + "(" + timer.get()
				+ "ms)");
	}
	...
	public void insertAuctionBoard(final L1AuctionBoardTmp board) {
		...
		try {
			con = DBCharacter.get().getConnection();
			pstm = con.prepareStatement("INSERT INTO `server_board_auction` SET " +
	...
	public void deleteAuctionBoard(final int houseId) {
		...
		try {
			...
			pstm = con.prepareStatement(
					"DELETE FROM `server_board_auction` WHERE `house_id`=?");
