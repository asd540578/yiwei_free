...
public class HouseTable implements HouseStorage {
	...
	public void load() {
		...
		try {
			...
			pstm = con.prepareStatement(
					"SELECT * FROM `server_house` ORDER BY `house_id`");
			...
			while (rs.next()) {
				...
				boolean is_on_sale = rs.getInt("is_on_sale") == 1 ? true : false;
				boolean is_purchase_basement = rs.getInt("is_purchase_basement") == 1 ? true : false;
	...
	public void updateHouse(final L1House house) {
		...
		try {
			con = DBCharacter.get().getConnection();
			pstm = con.prepareStatement(
					"UPDATE `server_house` SET `house_name`=?,`house_area`=?,`location`=?, " +
