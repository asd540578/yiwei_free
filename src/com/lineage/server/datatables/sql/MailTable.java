...
public class MailTable implements MailStorage {
	...
	//private static final ArrayList<L1Mail> _allMail = new ArrayList<L1Mail>();
	...
	private void deleteMail(String receiver) {
		...
		try {
			...
			pstm = con.prepareStatement(
					"DELETE FROM `character_mail` WHERE `receiver`=?");
	...
	public void setReadStatus(final int mailId) {
		...
		if (mail != null) {
			...
			try {
				...
				if ((rs != null) && rs.next()) {
					pstm = con.prepareStatement(
							"UPDATE `character_mail` SET `read_status`=? WHERE `id`=" + mailId);
					pstm.setInt(1, 1);
					pstm.execute();
	...
	public void deleteMail(final int mailId) {
		...
		if (mail != null) {
			...
			try {
				con = DBCharacter.get().getConnection();
				pstm = con.prepareStatement(
						"DELETE FROM `character_mail` WHERE `id`=?");
	...
	@Override
	public void writeMail(final int type, final String receiver, final L1PcInstance writer,
			final byte[] text) {
