...
public class VIPTable implements VIPStorage {
	...
	private static final Map<Integer, Timestamp> _vipMap =
		new ConcurrentHashMap<Integer, Timestamp>();
	...
	private static void delete(final int objid) {
		...
		try {
			...
			ps = cn.prepareStatement(
					"DELETE FROM `character_vip` WHERE `char_obj_id`=?");
			ps.setInt(1, objid);
			ps.execute();
	...
	private static void updateOther(final int objId, final Timestamp other) {
		...
		try {
			cn = DBCharacter.get().getConnection();
			ps = cn.prepareStatement(
					"UPDATE `character_vip` SET `overtime`=? WHERE `char_obj_id`=?");
	...
	private static void addNewOther(final int objId, final Timestamp other) {
		...
		try {
			cn = DBCharacter.get().getConnection();
			ps = cn.prepareStatement(
					"INSERT INTO `character_vip` SET `char_obj_id`=?,`overtime`=?");
