...
public class DwarfForElfTable implements DwarfForElfStorage {
	...
	public void load() {
		...
		try {
			...
			pstm = con.prepareStatement(
					"SELECT * FROM `character_elf_warehouse`");
	...
	private static void errorItem(int objid) {
		...
		try {
			con = DBCharacter.get().getConnection();
			pstm = con.prepareStatement(
					"DELETE FROM `character_elf_warehouse` WHERE `id`=?");
			pstm.setInt(1, objid);
			pstm.execute();
	...
	private static void deleteItem(final String account_name) {
		...
		try {
			...
			ps = cn.prepareStatement(
					"DELETE FROM `character_elf_warehouse` WHERE `account_name`=?");
	...
	public void insertItem(final String account_name, final L1ItemInstance item) {
		...
		try {
			con = DBCharacter.get().getConnection();
			pstm = con.prepareStatement(
					"INSERT INTO `character_elf_warehouse` SET `id`=?," +
	...
	public void updateItem(final L1ItemInstance item) {
		...
		try {
			con = DBCharacter.get().getConnection();
			pstm = con.prepareStatement(
					"UPDATE `character_elf_warehouse` SET `count`=? WHERE `id`=?");
