...
public class DwarfForClanTable implements DwarfForClanStorage {
	...
	public void load() {
		...
		_log.info("載入血盟倉庫物件清單資料數量: " + _itemList.size() + "/" + i + "(" + timer.get()
				+ "ms)");
	}
	...
	private static void errorItem(int objid) {
		...
		try {
			con = DBCharacter.get().getConnection();
			pstm = con.prepareStatement(
					"DELETE FROM `clan_warehouse` WHERE `id`=?");
			pstm.setInt(1, objid);
			pstm.execute();
	...
	private static void deleteItem(final String clan_name) {
		...
		try {
			...
			ps = cn.prepareStatement(
					"DELETE FROM `clan_warehouse` WHERE `clan_name`=?");
	...
	public void insertItem(final String clan_name, final L1ItemInstance item) {
		...
		try {
			con = DBCharacter.get().getConnection();
			pstm = con.prepareStatement(
					"INSERT INTO `clan_warehouse` SET `id`=?," +
	...
	public void updateItem(final L1ItemInstance item) {
		...
		try {
			con = DBCharacter.get().getConnection();
			pstm = con.prepareStatement(
					"UPDATE `clan_warehouse` SET `count`=? WHERE `id`=?");
