...
public class BuddyTable implements BuddyStorage {
	...
	private static final Map<Integer, ArrayList<L1BuddyTmp>> _buddyMap =
		new HashMap<Integer, ArrayList<L1BuddyTmp>>();
	...
	private static void delete(final int objid) {
		...
		try {
			...
			"DELETE FROM `character_buddys` WHERE `char_id`=?");
			ps.setInt(1, objid);
			ps.execute();
	...
	private static void delete2(final int buddy_id) {
		...
		try {
			cn = DBCharacter.get().getConnection();
			ps = cn.prepareStatement(
			"DELETE FROM `character_buddys` WHERE `buddy_id`=?");
