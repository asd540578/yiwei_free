...
public class FurnitureSpawnTable implements FurnitureSpawnStorage {
	...
	private static final Map<Integer, L1Furniture> _furnitureList = 
		new HashMap<Integer, L1Furniture>();
	...
	public void load() {
		...
		try {
			...
			ps = cn.prepareStatement(
					"SELECT * FROM `character_furniture`");
			rs = ps.executeQuery();
	...
	private static void delFurniture(final int objid) {
		...
		try {
			cn = DBCharacter.get().getConnection();
			pm = cn.prepareStatement(
					"DELETE FROM `character_furniture` WHERE `item_obj_id`=?");
			pm.setInt(1, objid);
			pm.execute();
	...
	public void insertFurniture(final L1FurnitureInstance furniture) {
		...
		try {
			...
			cn = DBCharacter.get().getConnection();
			ps = cn.prepareStatement(
					"INSERT INTO `character_furniture` SET `item_obj_id`=?,`npcid`=?,`locx`=?,`locy`=?,`mapid`=?");
