...
public class ClanTable implements ClanStorage {
	...
	public void load() {
		{
			...
			try {
				...
				ps = cn.prepareStatement(
						"SELECT * FROM `clan_data` ORDER BY `clan_id`");
		...
		for (final L1Clan clan : AllClan) {
			...
			try {
				cn = DBCharacter.get().getConnection();
				ps = cn.prepareStatement(
						"SELECT `char_name` FROM `characters` WHERE `ClanID`=?");
	...
	public L1Clan createClan(final L1PcInstance player, final String clan_name) {
		...
		try {
			cn = DBCharacter.get().getConnection();
			ps = cn.prepareStatement(
					"INSERT INTO `clan_data` SET `clan_id`=?,`clan_name`=?," +
	...
	public void updateClan(final L1Clan clan) {
		...
		try {
			cn = DBCharacter.get().getConnection();
			ps = cn.prepareStatement(
					"UPDATE clan_data SET `clan_id`=?,`leader_id`=?," +
