...
public class CharItemsTimeTable implements CharItemsTimeStorage {
	...
	//private static final Map<Integer, Timestamp> _timeMap = new HashMap<Integer, Timestamp>();
	...
	private static void delete(final int objid) {
		...
		try {
			...
			ps = cn.prepareStatement(
					"DELETE FROM `character_items_time` WHERE `itemr_obj_id`=?");
			ps.setInt(1, objid);
			ps.execute();
	...
	public void addTime(final int itemr_obj_id, final Timestamp usertime) {
		...
		try {
			...
			ps = co.prepareStatement(
					"INSERT INTO `character_items_time` SET `itemr_obj_id`=?,`usertime`=?");
