...
public class SpawnBossTable implements SpawnBossStorage {
	...
	public void load() {
		...
		try {
			...
			pstm = con.prepareStatement("SELECT * FROM `server_boss`");
			...
			while (rs.next()) {
				...
				if (temp1 == null) {
					...
					/*if (temp1.getTransformId() != -1) {
						...
						final L1Npc temp2 = NpcTable.get().getTemplate(temp1.getTransformId());
						temp2.set_boss(true);
						if (temp2.getTransformId() != -1) {
							// 加入BOSS ID 清單
							_bossId.add(new Integer(temp2.getTransformId()));
							final L1Npc temp3 = NpcTable.get().getTemplate(temp2.getTransformId());
							temp3.set_boss(true);
						}
					}*/
					...
					if ((locx2 < locx1) && locx2 != 0) {
						_log.error("server_boss : locx2 < locx1:"+id);
						continue;
					}

					if ((locy2 < locy1) && locy2 != 0) {
						_log.error("server_boss : locy2 < locy1:"+id);
						continue;
					}
	...
	public void upDateNextSpawnTime( final int id, final Calendar spawnTime) {
		...
		try {
			...
			con = DBCharacter.get().getConnection();
			pstm = con.prepareStatement("UPDATE `server_boss` SET `next_spawn_time`=? WHERE `id`=?");
