...
public class TownTable implements TownStorage {
	...
	public String totalContribution(final int townId) {
		...
		try {
			con = DBCharacter.get().getConnection();
			pstm1 = con.prepareStatement("SELECT `objid`, `char_name` FROM " +
			...
			pstm2 = con.prepareStatement("SELECT SUM(`Contribution`) AS " +
