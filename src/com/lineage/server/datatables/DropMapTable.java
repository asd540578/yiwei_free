...
public class DropMapTable {
	...
	public void load() {
		final Map<Integer, HashMap<Integer, ArrayList<L1DropMap>>> droplists = this.allDropList();
	...
	private Map<Integer, HashMap<Integer, ArrayList<L1DropMap>>> allDropList() {
		...
		final Map<Integer, HashMap<Integer, ArrayList<L1DropMap>>> droplistMap = 
			new HashMap<Integer, HashMap<Integer, ArrayList<L1DropMap>>>();
		...
		try {
			...
			while (rs.next()) {
				...
				if (check_item(itemId)) {
					final L1DropMap drop = new L1DropMap(mobId, mapid, itemId, min, max, chance);
	...
	private static void errorItem(int itemid) {
		...
		try {
			...
			pstm = con.prepareStatement(
					"DELETE FROM `droplist_map` WHERE `itemid`=?");
