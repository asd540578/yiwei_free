...
public class SceneryTable {
	...
	public void load() {
		...
		_log.info("載入景觀設置資料數量: " + _sceneryList.size() + "(" + timer.get()
				+ "ms)");
		this.satrt();
	}
	...
	public void storeScenery(final String note, final int gfxid, final int locx, final int locy
			, final int heading, final int mapid, final String html) {
		Connection con = null;
		PreparedStatement pstm = null;
		try {
			con = DBData.get().getConnection();
			pstm = con.prepareStatement(
					"INSERT INTO `spawnlist_scenery` SET `note`=?,`gfxid`=?," +
