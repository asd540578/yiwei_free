	...
	public void load() {
		...
		try {
			...
			while (rs.next()) {
				...
				final boolean is_spawn = rs.getBoolean("is_spawn");// true:一般召喚 false:副本用召喚

				final int npc_templateid = rs.getInt("npc_templateid");
				final L1Npc template1 = NpcTable.get().getTemplate(npc_templateid);
	...
	public static void delete(final int npc_id) {
		...
		try {
			...
			ps = cn.prepareStatement(
					"DELETE FROM `server_quest_spawn` WHERE `npc_templateid`=?");
			ps.setInt(1, npc_id);
			ps.execute();
