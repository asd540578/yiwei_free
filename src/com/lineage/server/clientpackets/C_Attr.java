		...
		try {
			...
			case 325: // 你想叫牠什麼名字？ 
				...
				if (pc.is_rname()) {
					...
					if (pc.getInventory().consumeItem(41227 , 1)) { // 刪除 更名卷

						World.get().removeObject(pc);// 移出世界

						final byte type = 1; // 名稱變更
						LogChangeReading.get().logArchive(type, pc.getId(), pc.getName(), name,
								pc.getNetConnection().getIp().toString()); // 更名紀錄
