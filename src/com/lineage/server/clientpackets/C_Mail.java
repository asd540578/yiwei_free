			...
			if (pc.getInventory().consumeItem(40308, 1000)) {
				if (clan != null) {
					for (final String name : clan.getAllMembers()) {
						...
						MailReading.get().writeMail(TYPE_CLAN_MAIL, name, pc, text);
						final L1PcInstance clanPc = World.get().getPlayer(name);
						if (clanPc != null) { // オンライン中

							final ArrayList<L1Mail> mails = MailReading.get().getMails(name);
							if (!mails.isEmpty()) {
	...
	private void clientPackD(L1PcInstance pc, int type, String receiverName, byte[] textr) {
		try {
			...
			if (pc.getInventory().consumeItem(40308, 50)) {
				if (receiver != null) { // 連線
					if (MailReading.get().getMailSizeByReceiver(receiverName, TYPE_NORMAL_MAIL) >= 20) {
						pc.sendPackets(new S_Mail(type));
						return;
					}
					MailReading.get().writeMail(TYPE_NORMAL_MAIL, receiverName, pc, textr);
					if (receiver.getOnlineStatus() == 1) {

						final ArrayList<L1Mail> mails = MailReading.get().getMails(receiverName);
						if (!mails.isEmpty()) {
							receiver.sendPackets(new S_Mail(mails, TYPE_NORMAL_MAIL));
						}
					}
