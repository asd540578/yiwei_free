 ...
 * 顯示物件ID
 * @author dexc
 * DELETE FROM `commands` WHERE `name`='id';
		...
		try {
			for (final L1Object object : pc.getKnownObjects()) {
				...
				if (object instanceof L1TrapInstance) {
					...
					pc.sendPackets(new S_Chat(object, "XY:"+tg.getX() + "/" + tg.getY(), 0));
					continue;
				}
				// 寵物
				...
				if (object instanceof L1DollInstance) {
					...
					pc.sendPackets(new S_Chat(object, "Over Time:"+tg.get_time(), 0));
					continue;
				}
				// Effect(技能物件)
				...
				if (object instanceof L1MonsterInstance) {
					...
					pc.sendPackets(new S_Chat(object, "NpcId:" + tg.getNpcId(), 0));
					continue;
				}
				// NPC
