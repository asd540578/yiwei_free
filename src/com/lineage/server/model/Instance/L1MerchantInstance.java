	...
	public void onTalkAction(final L1PcInstance player) {
		...
		if (talking != null) {
			...
			/*} else if (npcid == 70904) { // クプ
				if (player.isDarkelf()) {
					if (quest.get_step(L1PcQuest.QUEST_LEVEL45) == 1) { // ブルディカ同意済み
						htmlid = "koup12";
					}
				}*/
				...
				if (player.getQuest().get_step(L1PcQuest.QUEST_DESIRE) == L1PcQuest.QUEST_END) {
					...
					htmlid = "minicod05";
				} else if ((player.getQuest().get_step(L1PcQuest.QUEST_SHADOWS) == L1PcQuest.QUEST_END // 影の神殿側クエスト終了
				)
				...
				if (player.getQuest().get_step(L1PcQuest.QUEST_SHADOWS) == L1PcQuest.QUEST_END) {
					...
					htmlid = "minitos05";
				} else if ((player.getQuest().get_step(L1PcQuest.QUEST_DESIRE) == L1PcQuest.QUEST_END // 欲望の洞窟側クエスト終了
				)
	...
	private void createRuler(final L1PcInstance pc, final int attr, final int sealCount) {
		...
		if (item != null) {
			pc.sendPackets(new S_ServerMessage(143,
					this.getNpcTemplate().get_name(), item.getLogName())); // \f1%0が%1をくれました。
		}
	}
