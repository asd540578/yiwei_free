...
public class L1PetInstance extends L1NpcInstance {
	...
	public boolean noTarget() {
		...
		default:
			if ((this._petMaster != null) && (this._petMaster.getMapId() == this.getMapId())) { // ●
				// 主人を追尾
				if (this.getLocation().getTileLineDistance(_petMaster.getLocation()) > 2) {
					if (_npcMove != null) {
	...
	public L1PetInstance(final int npcid, final L1PcInstance master, final int itemid) {
		...
		this._itemObjId = itemid;
		this._type = 
			PetTypeTable.getInstance().get(npcid);
	...
	public void receiveDamage(final L1Character attacker, final int damage) {
		...
		if (this.getCurrentHp() > 0) {
			if (damage > 0) { // 回復の場合は攻撃しない。
				this.setHate(attacker, 0); // ペットはヘイト無し
	...
	public void collect(final boolean isDepositnpc) {
		...
		for (final L1ItemInstance item : items) {
			...
			// 容量重量確認
			if (this._petMaster.getInventory().checkAddItem(item, item.getCount()) == L1Inventory.OK) {
	...
	public void setTarget(final L1Character target) {
		if ((target != null) && (
				(this._currentPetStatus == 1) || // 攻擊
				(this._currentPetStatus == 2) || // 防禦
	...
	public void onAction(final L1PcInstance player) {
		...
		if (this.isSafetyZone() || player.isSafetyZone()) {// 安全區域中
			final L1AttackMode attack_mortion = new L1AttackPc(player, this); // 攻擊判斷
			attack_mortion.action();
			return;
		}
					...
					if (this._petMaster != null) {
						...
						if (this._petMaster.getLevel() >= pet.getLevel()) {
							...
							if (this._type != null) {
								...
								if (id != 0) {
									this.broadcastPacketX8(new S_NpcChat(pet, "$" + id));
								}
							}
						}
	...
	public void onItemUse() {
		if (!this.isActived()) {
			this.useItem(USEITEM_HASTE, 100); // １００％の確率でヘイストポーション使用
		}
		if (this.getCurrentHp() * 100 / this.getMaxHp() < 40) { // ＨＰが４０％きったら
			this.useItem(USEITEM_HEAL, 100); // １００％の確率で回復ポーション使用
		}
	}
