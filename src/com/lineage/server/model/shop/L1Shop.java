...
public class L1Shop {
	...
	public void buyItems(final L1ShopSellOrderList orderList) {
		final L1PcInstance pc = orderList.getPc();
		final L1PcInventory inv = pc.getInventory();
		int totalPrice = 0;
		for (final L1ShopSellOrder order : orderList.getList()) {

			final L1ItemInstance sellitem = inv.getSellItem(order.getItem().getTargetId());

			if (sellitem == null)
				continue;

			final long count = inv.removeSellItem(sellitem, order.getCount());

			final long prices = order.getItem().getAssessedPrice() * count;

			totalPrice += prices;

			LogItemSellNpcReading.get().logArchive(pc, sellitem, count, prices); // 賣NPC紀錄
		}
