...
public class L1TownLocation {
	...
	public static final boolean isGambling(final L1PcInstance pc) {
		final int x = pc.getX();
		final int y = pc.getY();
		final short m = pc.getMapId();
		return isGambling(x, y, m);
	}

	public static final boolean isGambling(final int x, final int y, final short map) {
		...
		if (map == 4) {
			...
			if (((x > 33528) && (x < 33427)) && ((y > 32848) && (y < 32871))) {
				//System.out.println("000000000003");
				return true;
			}
		}
		return false;
	...
	public static int[] getGetBackLoc(final int town_id) { // town_idから帰還先の座標をランダムに返す
		...
		case TOWNID_SILVER_KNIGHT_TOWN: // SKT
		...
		case TOWNID_SILENT_CAVERN: // 沈黙の洞窟
		...
		case TOWNID_RESISTANCE: // レシスタンス村
		...
		case TOWNID_RECLUSE_VILLAGE: // 隠れ里
