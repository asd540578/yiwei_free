...
import log.datatables.lock.LogWorldTreasureReading;
	...
	private class DropShareR implements Runnable {
									...
									if (acquisitor instanceof L1PcInstance) {
										...
										if (ConfigBoxMsg.ISMSG) {
											if (ItemMsgTable.get().contains(item.getItemId())) {
												...
												LogWorldTreasureReading.get().logArchive(player, item, _npc,
														item.getCount()); // 打寶公告
											}
										}
									}
		...
		private void set_power(L1PcInstance tgpc, L1ItemInstance item) {
			if (PowerItemSet.START) {
				...
				int r = 5;
				if (tgpc.getInventory().checkEquipped(37001)) {// 野外打怪道具 增設  5% 成功凹槽
					r = 50;
				} else if (tgpc.getInventory().checkEquipped(37002)) {// 野外打怪道具 增設  10%成功凹槽
					r = 100;
				} else if (tgpc.getInventory().checkEquipped(37003)) {// 野外打怪道具 增設  15%成功凹槽 
					r = 150;
				} else if (tgpc.getInventory().checkEquipped(37004)) {// 野外打怪道具 增設  20%成功凹槽  (boss取得)
					r = 200;
				} else if (tgpc.getInventory().checkEquipped(37005)) {// 野外打怪道具 增設  100%成功凹槽 (商城)
					r = 1000;
				}
