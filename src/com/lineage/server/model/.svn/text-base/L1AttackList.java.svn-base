...
public class L1AttackList {
	...
	// 技能增加閃避
	//protected static final HashMap<Integer, Integer> SKU3 = new HashMap<Integer, Integer>();
	...
	public static void load() {
		...
		for (Integer bossid : SpawnBossReading.get().bossIds()) {
			Integer[] ids = new Integer[]{
					new Integer(FOG_OF_SLEEPING),// 沉睡之霧
		...
		// 技能增加閃避
		/*SKU3.put(new Integer(UNCANNY_DODGE), new Integer(-10));// 2012-1-29(-8)
		SKU3.put(new Integer(MIRROR_IMAGE), new Integer(-5));// 原始 -5
		SKU3.put(new Integer(AQUA_PROTECTER), new Integer(-5));// 原始 -5
		SKU3.put(new Integer(RESIST_FEAR), new Integer(+25));// TEMP 2010/05/19(5)
		SKU3.put(new Integer(DRAGON2), new Integer(-1));// 地 攻擊迴避提升 石化耐性+3
		SKU3.put(new Integer(DRAGON5), new Integer(-5));// 生命-物理攻擊迴避率+10% 魔法傷害減免+50 魔法暴擊率+1 額外攻擊點數+2 防護中毒狀態
		SKU3.put(new Integer(DRAGON6), new Integer(-5));// 誕生-物理攻擊迴避率+10% 魔法傷害減免+50 暗黑耐性+3
		SKU3.put(new Integer(DRAGON7), new Integer(-5));// 形象-物理攻擊迴避率+10% 魔法傷害減免+50 魔法暴擊率+1 支撐耐性+3*/
		...
		testStr.putAll(strDmg);
		for (Integer integer : testStr.keySet()) {
		...
		testDex.putAll(dexDmg);
		for (Integer integer : testDex.keySet()) {
