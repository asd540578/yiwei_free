 ...
 *
 */
public class LogItemDeleteTable implements LogItemStorage {

	private static final Log _log = LogFactory.getLog(LogItemDeleteTable.class);
	...
	public void logArchive(final L1PcInstance pc , final L1ItemInstance item  ,  final long removeCount )
	{
		...
		try {
			...
			ps = cn.prepareStatement(
					"INSERT INTO `other_item_del` SET " + " `item_objid`=? , `item_name`=? , `char_id`=? ,`character_items`=? , `datetime`=SYSDATE() , `ip`=?");
