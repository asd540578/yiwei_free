...
public class Letter extends ItemExecutor {
	...
	private boolean writeLetter(final int itemId, final L1PcInstance pc, final int letterCode,
		...
		if (this.sendLetter(pc, letterReceiver, item, true)) {
			this.saveLetter(item.getId(), letterCode, pc.getName(), letterReceiver,
					letterText);
		} else {
			return false;
		}
		...
		if (target != null) {
			...
			if (CharacterTable.doesCharNameExist(name)) {
				try {
					...
					CopyOnWriteArrayList<L1ItemInstance> list = CharItemsReading.get().loadItems(objid);
	...
	private void saveLetter(final int itemObjectId, final int code, final String sender,
			final String receiver, final byte[] text) {
