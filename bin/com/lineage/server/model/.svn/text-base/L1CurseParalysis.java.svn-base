...
public class L1CurseParalysis extends L1Paralysis {
	...
	private class ParalysisDelayTimer extends Thread {
		...
		public void run() {
			...
			try {
				Thread.sleep(L1CurseParalysis.this._delay); // 麻痺するまでの猶予時間を待つ。
			...
			GeneralThreadPool.get().execute(L1CurseParalysis.this._timer); // 麻痺計時開始
	...
	private class ParalysisTimer extends Thread {
		...
		public void run() {
			...
			L1CurseParalysis.this.cure(); // 解呪処理
		}
	}
