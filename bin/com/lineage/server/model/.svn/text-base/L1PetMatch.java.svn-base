...
public class L1PetMatch {
	...
	public synchronized boolean enterPetMatch(final L1PcInstance pc, final int amuletId) {
		...
		final L1PetMatchReadyTimer timer = new L1PetMatchReadyTimer(petMatchNo, pc,
				pet);
		timer.begin();
		return true;
	}
	...
	public void startPetMatch(final int petMatchNo) {
		...
		final L1PetMatchTimer timer = new L1PetMatchTimer(this._pet1[petMatchNo],
				this._pet2[petMatchNo], petMatchNo);
		timer.begin();
	}
	...
	private void qiutPetMatch(final int petMatchNo) {
		final L1PcInstance pc1 = World.get()
		.getPlayer(this._pc1Name[petMatchNo]);
		...
		final L1PcInstance pc2 = World.get()
		.getPlayer(this._pc2Name[petMatchNo]);
	...
	public class L1PetMatchReadyTimer extends TimerTask {
		...
		public L1PetMatchReadyTimer(final int petMatchNo, final L1PcInstance pc,
		...
		public void run() {
			try {
				for (;;) {
					...
					if (L1PetMatch.getInstance().setPetMatchPc(this._petMatchNo,
							this._pc, this._pet) == L1PetMatch.STATUS_PLAYING) {
	...
	public class L1PetMatchTimer extends TimerTask {
		...
		public L1PetMatchTimer(final L1PetInstance pet1, final L1PetInstance pet2,
				final int petMatchNo) {
			this._pet1 = pet1;
			this._pet2 = pet2;
			this._petMatchNo = petMatchNo;
		}
